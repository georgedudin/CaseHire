...
| Stamp wall (✗/✓ slams) | 12 | slam grammar otherwise reserved to: 8 leak flag, 13 epitaph (SUPERSEDED 2026-06-13: 11's payback verdict no longer slams — it RISES; see §2.3) |
| Dual-fill matrix + epitaph overlay | 13 | — |

**Stamp grammar reservation:** notary-slam (scale 1.4–1.6→1 + rotation settle + jolt) appears ONLY at: slide 8 leak flag, slide 12 ✗/✓ wall, slide 13 epitaph lines. Slide 3's Shopify/Coinbase chips fade in with rotation PRE-SET; slide 10's SOM goal fades up. ~~slide 11 payback verdict~~ — SUPERSEDED 2026-06-13: slide 11's verdict now RISES (autoAlpha + y, no slam) under the casino lock-in redesign; it has left the notary list.
...
| 10→11 | рынок → деньги | Dived state: flame-ringed «20–35 млн ₽» | Money register continuity: flame ring → «Кандидат» card ring; ru-RU numerals carry |
| 11→12 | деньги → конкуренты | Locked good card + flame-struck «1,5 млн ₽» + risen verdict line (SUPERSEDED 2026-06-13: was a tipped beam + slammed payback stamp) | Money/flame continuity: 11's locked flame card + struck cost → 12's ✗/✓ verdict wall. The stamping-cadence handoff is softened — 11 no longer slams; 12 opens the notary register itself. |
| 12→13 | конкуренты → финал | Full table, dimmed rivals, lit pentagon + verdict (auto-chained) | Closed pentagon → line forward: 13's entrance draws the timeline; flame travels verdict → «Агенты» ring |
...
> **AMENDMENT 2026-06-13 (ui-fixes batch 2 — step-card declutter).** The three step cards previously spent **orange three times per card** (the flame numeral + a flame-text plaque + the «Никогда.» flame underline) and carried four type levels, double-bold body lines, and in-card `→` arrows that collided with the inter-card chevron grammar. Orange is the deck's scarcest resource. The card's orange budget is now **the numeral only** (plus the rec dot on card 3; the transient ember flash on «30 секунд» during the build stays — it self-clears). This **supersedes** the following in this section:
> - **Intent & copy** (and the **Layout — desktop** «plaque chip … in `line-strong` border with flame text» clause): plaques now render as **`text-mute`** (no `line-strong` pill-border wrapper at any viewport) — `#a1a1aa`, verified `rgb(161, 161, 170)`. Card-1 detail loses its bold on «1–3 строки» and its `→` arrows become middots: «стек · "как мы работаем" · бизнес-контекст · 1–3 строки про задачу позиции». Card-2 tagline arrows become middots: «ссылка · **30 секунд** · веб-IDE» (the `[data-chip-30s]` `<b>` keeps `font-semibold text-paper` — it is the ember-flash build target + breathe idle target). One body accent per type-zone: card 1 keeps «один раз» (tagline) **and** «Никогда.» (plaque, now a plain `font-semibold text-paper` span — the wrapper span and SVG are gone).
> - **«Никогда.» flame underline is deleted entirely** — supersedes **Animation-timeline beat 3** («…«Никогда.» word gets a flame underline DrawSVG» → just «both plaques stamp (scale 1.12→1, opacity)») and removes **«underline»** from the **Implementation-notes** DrawSVG list («DrawSVG (chevrons, underline, schema)» → «DrawSVG (chevrons, schema)»). The plaque stamp (scale 1.12→1) and dormant scale 1.12 are unchanged.
> - Spacing rhythm tightened to mt-2 / mt-1.5 / mt-2 (tagline / detail / plaque), detail `leading-relaxed`. Build count stays 18 (the underline tweens lived in `entrance`, not `build`); `[data-plaque-underline]` no longer exists in the DOM. Verify: `[data-plaque-underline]` count = 0; every `[data-plaque]` computed color `rgb(161, 161, 170)` at settled **and** built, mobile **and** lg; «Никогда.» semibold paper with no wrapper/SVG sibling; card-1 detail has no `font-semibold text-paper` token; no `→` inside any `[data-step]`; `[data-chip-30s]` `<b>` survives; no `[data-step]` horizontal overflow at 375.

---

### Slide 07 — Карточка кандидата · «Evidence Boot»
**Intent & copy.** The product boots and operates itself: a ranked feed reorders on evidence, the matrix fills, the digest stamps. The judge must feel "this dashboard is alive and nobody is touching it". On-screen strings (verbatim): headline **«Тимлид получает не код.»**; nine matrix axes **понимание контекста · планирование · точность промптов · калибровка ИИ · безопасность команд · проверка · восстановление · артикуляция · безопасность данных**; digest line **«⚠ вставил файл с API-ключом во внешний чат · 14:32»**; button **«Запись сессии»**; bottom caption **«На доказательствах. Не на догадках.»** (from the speech close — visible early so judges read while the speaker builds to it). Product-chrome eyebrow «КейсПодбор · оценка» comes from the existing mockup.
...
5. t=1.30 (overlapping), matrix card in (0.4s); then bars fill **axis by axis**: inner fill span `scaleX 0→score/100`, `transform-origin:left`, 0.5s each, stagger 0.18, `power3.out` — ninth bar (безопасность данных, flame-colored) lands ≈3.2s; header average counts to its /100 value in parallel.
...
**Transition out.** The digest line is the verbal and visual hinge: the speaker reads «вставил файл с API-ключом…» verbatim, judges' eyes are parked on the amber ⚠ as they scroll. Slide 8 opens its dual-chat with the same ⚠ glyph vocabulary, paying it off in red. Slide 7 deliberately never uses full red — the data-safety bar is flame, ⚠ is amber — so slide 8's red flare lands at full force.
...
10. t=1.3, 1.1s, power3.in + tiny bounce — matrix «Безопасность данных»: score rolls 89→18, bar scaleX 0.89→0.18 (transform-origin left), color trust→leak; matrix average rolls down in parallel.
...
**Product-UX elements.** `ProcessMatrix` with `leakLabel="Безопасность данных"` (extended with a `score-roll` ref hook + scaleX bars instead of width transition); bespoke `<Bubble>` dual-chat from existing 06-data-trap (copy reused verbatim); new `FileChip`, `BoundaryLine` (SVG), `FlareOverlay` components. The matrix demonstrates the actual product output: a measurable data-safety score crashing in real time.
...
**Intent & copy.** Make TAM→SOM honest and physical: every circle is area-true relative to its neighbor, and the SOM is deliberately almost invisible at first — a pulsing ember dot. The judge's takeaway: "they didn't inflate the market; they zoomed into their slice." On-screen strings (verbatim): «Рынок есть. И он растёт на 38% в год.» · «99,3 млрд ₽» + «российский HR-tech, 2024, +38% · Smart Ranking» · «3,85 млрд ₽» + «наш сегмент: оценка и развитие, +38%» · «43%» компаний уже используют ИИ в HR · «27%» тестируют для 2026 · «(Известия)» · «400–800 компаний» «(10–50 джунов в год)» · «средний чек 400–900 тыс ₽/год» · «потолок клина 160–720 млн ₽» · «цель на 24 мес: 20–35 млн ₽» · «50–80 платящих»
...
**Transition out.** The slide leaves in its dived state: a flame-ringed circle holding «20–35 млн ₽». Slide 11 opens with money mechanics; the ember's flame accent hands off directly to the «Кандидат» card's flame border — same hue, same corner of the screen. SOM revenue → tariffs that produce it.

**Reduced motion.** Pin/build disabled (Scene matchMedia guard); render the dived composite statically: клин ring + flame SOM circle + full left-column stack showing all numbers at final values (99,3 / 3,85 / 43 / 27 / клин rows / SOM stamp). No loops, no ember pulse.
...
- Build = shared `usePinnedBuild`. Frozen state: dived клин view.
...
> right «~1 000» (text-flame) «кандидатов». Each card carries a
> pre-rendered radial-flame `[data-card-glow]` (opacity-only). Below the grid:
> `[data-verdict]` «один предотвращённый найм = оценка ~1 000 кандидатов» (13/16px) +
> `[data-shrm]` «SHRM: замена = 100% годовой зарплаты». **The long stamp
> sentence «Один предотвращённый плохой найм окупает „Команду“ на годы»
...
>   §2.5). The beam ±0.3°/±0.4° sways are deleted. Sheen + «$4,5» breathe
...
**Intent & copy.** The slide must read as accounting, not marketing: a machine that visibly earns. Three claims in speech order — model, cost, payback — each with its own metaphor: tariff cards (model), a stacking cost bar (cost), a casino lock-in (payback). On-screen copy, verbatim: headline «Платят за завершённую сессию. Не за кресла.»; cards «Пилот» — бесплатно (первые 20 кандидатов) / «Кандидат» — 1 500 ₽ за оценённого кандидата · оплата по факту + chip «основной» / «Энтерпрайз» — по запросу + локальное развёртывание; value anchor «в 2–3 раза дешевле часа живого техсобеса ($35–60/час)»; cost strip «Себестоимость сессии $4,5» with segments «ИИ-напарник и оценка $2,6» · «инфраструктура $0,9» · «ручной разбор и буфер $1,0» and «Валовая маржа ~72%»; lock-in cards «1,5 млн ₽ — замена плохого найма» / «~1 000 — кандидатов»; verdict «один предотвращённый найм = оценка ~1 000 кандидатов»; source line «SHRM: замена = 100% годовой зарплаты».

**Layout — desktop (1366/1920).** Tight vertical ladder, scene-content block padding overridden to `py-8 lg:py-10` (the default 144px ladder will not fit this density at 768px tall). Zone A: headline, `--text-h1` clamp (~44px at 1366), centered, one line at 1920 / two at 1366. Zone B (t+24px): 3-up tariff grid, `gap-4`, cards ~280×190px in `bg-fog` `border-line` rounded-2xl; «Кандидат» gets `z-10`, scale 1.04, flame SVG ring, top-right chip «основной» (flame bg, ink text); value-anchor line under the grid. Zone C (t+20px): cost machine — a full-width 28px-tall track (`bg-fog`, `border-line`) with three stacked segments (sterile / glass / mute), labels beneath each, total «$4,5» in `--text-h2` flame at the right end; «Валовая маржа ~72%» readout right-aligned under the track in `mute`. Zone D (t+20px): casino lock-in, ~116px tall — two equal fog cards in a 2-col grid: left card «1,5 млн ₽ — замена плохого найма» (sterile tone), right card «~1 000 — кандидатов» (flame tone, the locked winner). Verdict line + SHRM source sit below, empty until the build. Total ≈ 700px at 1366×768 — fits with breathing room at 1080.

**Layout — mobile (375 × ~660svh).** Headline ~26px, two lines. Tariffs become a compact card stack (~165px wide): name 13px, price 16px semibold, limits collapsed to one 11px line («за оценённого кандидата · оплата по факту»); «Энтерпрайз» spans full width. Cost track shrinks to 18px tall; segment labels move below as a 3-chip legend, margin line merges into the legend row. Lock-in cards stay a 2-col grid (`max-w-[300px]`) and compress to 2-line chips; verdict wraps to two lines at 13px. Nothing is cut — only compressed; SHRM source line is the single sacrifice if 660svh is exceeded (moves into the verdict block as a 10px footer).
...
3. t=1.3, 0.6s — «Кандидат» feature beat: card `scale 1→1.04` (back.out(1.4)), flame ring draws via DrawSVG around an SVG rect overlay, chip «основной» pops `scale 0.8→1`.
4. t=1.8 — cost machine: empty track fades in 0.2s; segments stack left→right via `scaleX` (`transform-origin:left`): ai 0.25s → infra 0.35s → buffer 0.3s, each label fading under its segment; «$4,5» count-up finishes t=2.9; margin readout fades t=2.8–3.1.
5. t=3.2, 0.5s — lock-in zone appears in dormant pre-state: both cards neutral at 40% opacity, no verdict. **Stable and fully readable by t≈3.8s.**
...
**Idle state.** A skewed low-opacity highlight div translates across the «Кандидат» ring every ~6s (transform-only sheen); cost-bar total «$4,5» breathes opacity 1→0.85 on an 4s sine; pre-build lock-in zone holds at 40% opacity, both cards neutral, hinting it is waiting.

**Build steps.** ONE, on the speech beat «И ключевая цифра» (~t+45s of the 60s slide). Scene pins on `lg+` with one internal snap. Gesture/→ advances the **casino lock-in** (full beats in the SUPERSEDED Zone-D banner at the top of this section): (a) zone wakes 0.4→1, «1,5 млн ₽» count-up rolls during the flicker, both cards dip to 0.55; (b) a slowing roulette — the highlight alternates bad→good→bad→good→bad at decelerating gaps and LOCKS on the cheaper «~1 000 кандидатов» card (scale pop 1→1.04→1); (c) the losing card dims to 0.45, a flame strike draws across «1,5 млн ₽», the verdict «один предотвращённый найм = оценка ~1 000 кандидатов» RISES (autoAlpha + y — NO notary slam), SHRM line fades in beneath. Post-build idle: the locked flame glow breathes. Next gesture leaves the slide. Below `lg` and under reduced motion, the build plays automatically as timeline beats 7–9 right after entrance (Scene pin gate already handles this).
...
**Transition out.** The lock-in is the deliberate motif handoff: slide 11's locked flame card + struck «1,5 млн ₽» hand the flame/stamping register to slide 12's verdict table, which stamps ✗/✓ — 11 no longer slams (the verdict RISES), so slide 12 opens the notary register itself.

**Reduced motion.** Everything renders instantly in the FINAL post-build state: headline with strike-through, cards with «Кандидат» featured, full cost bar with «$4,5», Zone D = locked good card + glow, dimmed bad card with the «1,5 млн ₽» strike drawn, verdict + SHRM visible. No loops, no sway, no pin.
...
**Transition in.** From slide 11's locked payback card: deck-level crossfade; the headline's word-stagger entrance picks up 11's flame/stamping cadence (locked lock-in card → «Мы не одни…» stamp). No shared elements — clean cut keeps 11's machine self-contained.
...
**Transition in.** Slide 12 ends on five flame ✓ locks; slide 13's first flame accent is the «Агенты» ring — the flame literally travels from verdict to horizon. Standard snap; nothing pre-animates during travel.
...
- «Агенты» ring breathing idle is the sanctioned callback to slide 4's ring (§2.5).
...
1. **Slide 13 — milestone dots ride the spine (cross-browser).** *Superseded the original `align-top` approach 2026-06-13 (Safari).* The `[data-node-dot]` span is now a `block h-3 w-3` element. `inline-block` + `align-top` centered the dot on the spine in Blink (0.00px) but Safari's inline-block strut/baseline handling drifted the flame dot **above** the spine. As a `block`, the 12px box sits deterministically at the `<li>` content top in every engine; the spine is `top-[5px] h-[2px]` (y 5..7, center 6), so the box centers at exactly 6. The label's `mt-1.5` → `mt-5` replaces the line-box strut spacing the `block` removes, so labels do not move (Chrome label top 31.6px → 32px — sub-pixel). Verify: `|dotCenterY − spineCenterY| ≤ 1px` for all four dots at 375 / 1366 / 1920, measured at settled (Blink: 0.00px).
...
6. **Slide 11 — casino lock-in replaces the payback seesaw:** `payback-scale.tsx` is rewritten to render two equal fog cards (left «1,5 млн ₽» `[data-sum-bad]` + flame strike `[data-strike-sum]` / right «~1 000» — кандидатов), each with a radial-flame `[data-card-glow]`, and a risen `[data-verdict]` «один предотвращённый найм = оценка ~1 000 кандидатов» + `[data-shrm]`; the long stamp sentence and the beam/pans are gone. `11-monetization.tsx` rewires Zone D to `cardBad/cardGood/glows/goodGlow/sumBad/strikeSum/verdict/shrm` (+ a `token()` helper, `LINE`, `FLAME_HI = rgba(255,90,31,0.6)`), deletes `setZoneCommon`, and replaces the build with a **slowing-roulette flicker** (instant `.set` swaps at 0.45/0.60/0.75/0.95/1.25 → LOCK on the good card at 1.70 with a scale pop → bad dims to 0.45 → strike draws → verdict rises → SHRM fades). The locked-glow `breathe(goodGlow, 0.5, 0.62, 4)` replaces the deleted beam sways; `build.progress(1)` ≡ `setFrozen("built")` (all flicker beats are `.set`s). The per-candidate pricing model (Пилот free / Кандидат 1 500 ₽ / Энтерпрайз) + COGS $4,5 + ~72% gross are the verbatim Zone-B/C copy. Verified (`/tmp/deck-verify/ui-fix-s11.mjs`, 64/64 green): @1366/@1920 build-play AND @375 tap-build AND End→back frozen-parity all assert `[data-beam]` count = 0, good-card border `rgba(255, 90, 31, 0.6)`, good `[data-card-glow]` opacity ≥ 0.4, bad-card opacity 0.45, strike fully drawn, `[data-verdict]` text exact, `[data-sum-bad]` = «1,5 млн ₽»; settled state = zone 0.4, neutral cards, strike undrawn, verdict hidden; reduced-motion 21/21 green at 375/1366/1920; full forward walk = 18 gestures to a built finale; no horizontal overflow; no page errors; `npm run lint:deck` green.

7. **Slide rail — round keyboard-focus cue.** The global `:focus-visible` rule (§globals) sets `border-radius: 2px` + `outline-offset: 3px`, which forced the 8px round `ProgressRail` dots' focus indicator into a **square** (seen after click-then-arrow, when the clicked dot keeps focus and the next keypress flips on `:focus-visible`). A scoped, unlayered `nav[aria-label="Слайды"] button:focus-visible { border-radius: 9999px }` keeps the cue round — the square is gone and keyboard focus stays visible (a11y intact, WCAG 2.4.7). Verified: a Tab-focused rail dot computes `border-radius: 9999px`.
